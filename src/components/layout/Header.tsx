...
import type { SiteConfigData, SocialLinksData } from "@/lib/db/site-config";
import {
  ChevronRight,
  Phone,
  Mail,
...
export default function Header({
  ...
  socialLinks: propSocial,
}: {
  siteConfig?: SiteConfigData;
  socialLinks?: SocialLinksData;
}) {
  const config = propConfig || SITE_CONFIG;
  const social = propSocial || SOCIAL_LINKS;
  ...
  return (
    <header className="w-full relative z-50">
      ...
      <div
        className={`fixed top-0 left-0 right-0 z-50 bg-[#002F57] text-white transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-10 text-sm">
            ...
            <div className="flex items-center space-x-4">
              ...
              <a
                href={`https://wa.me/51${config.whatsapp}`}
                target="_blank"
          ...
          <div className="flex justify-between items-center h-20">
            ...
            <Link href="/" className="flex-shrink-0 group">
              <div className="flex items-center">
                <div
                  className={`relative overflow-hidden rounded-full aspect-square shadow-lg transition-all duration-300 group-hover:shadow-xl group-hover:scale-105 ${
                ...
                <span
                  className={`ml-3 text-lg font-semibold text-[#002F57] hidden sm:block transition-all duration-300 ${
            ...
            <div className="hidden lg:flex items-center space-x-1">
              {NAV_ITEMS.map((item) => (
                ...
                >
                  {item.name}
                  <span
                    className={`absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 bg-gradient-to-r from-[#49AE9C] to-[#002F57] transition-all duration-300 rounded-full ${
            ...
            {NAV_ITEMS.map((item, index) => (
              <Link
                ...
                className={`block py-3 px-4 text-gray-700 hover:text-[#002F57] hover:bg-white/40 font-medium transition-all duration-300 rounded-lg ${
                  pathname === item.href
                    ? "text-[#002F57] bg-white/40 font-semibold"
                    : ""
